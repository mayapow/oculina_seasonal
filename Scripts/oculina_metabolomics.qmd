...
#add data for general and species columns based on oculina_full
...
sym_chla <- sym_chla %>% rename_at('sample_ID', ~'id')
oculina_final <- oculina_clean %>% left_join(sym_chla, by = "id")
```
...
#and read back in saved version
